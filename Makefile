...
.PHONY: validate
validate:
	@for m in $(TF_TESTS); do terraform -chdir="$$m" init > /dev/null 2>&1; echo "$$m: "; terraform -chdir="$$m" validate; done
...
validate-ign:
	@(cd tests && \
	  ...
	  (ignition-validate output/result.ign && echo "√ output/result.ign: Success! The ignition configuration is valid."))
